...
from .shortcut import T_Callable, Wrapper, gen_subclass, decorate, ensure_cube_as_listener
...
def fetch_name(path: str = "name"):
    ...
    async def __wrapper__(app: Ariadne, result: AlconnaProperty):
        ...
        if t := arp.all_matched_args.get(path, None):
            return (
                t.display or (await app.getUserProfile(t.target)).nickname
                if isinstance(t, At)
                else t
            )
...
class MatchPrefix(Decorator):
    ...
    async def __call__(
        ...
        if not rest.__root__:
            raise ExecutionStop
        elem = rest.__root__[0]
        if isinstance(elem, Plain) and (res := self.pattern.validate(elem.text)).success:
            if self.extract:
                return MessageChain([Plain(str(res.value))])
            elem.text = elem.text[len(str(res.value)):].lstrip()
            return header + rest
        elif self.pattern.validate(elem).success:
            if self.extract:
                return MessageChain([elem])
            rest.__root__.remove(elem)
            return header + rest
        raise ExecutionStop


class MatchSuffix(Decorator):
    pre = True
